...
function tcs_hp_taxonomies_project() {
	...
	register_taxonomy( 'product_category', array('project','news'), $args2 );
}
...
function tcs_hp_project_additional_box_content($argpost){
?>
	...
	( function( $ ) {
		$(document).ready(function(){
			...
			$('#<?php echo get_post_meta($argpost->ID,'project_type',true);?>_div').show('fast');
...
<?php
	...
	echo '<select id="project_type" name="project_type">
			<option value="brand" '.(get_post_meta($argpost->ID,'project_type',true)=="brand"?"selected":"").'>Brand Activation</option>
			<option value="bespoke" '.(get_post_meta($argpost->ID,'project_type',true)=="bespoke"?"selected":"").'>Bespoke Solution</option>
			<option value="digital" '.(get_post_meta($argpost->ID,'project_type',true)=="digital"?"selected":"").'>Digital Platform</option>
		</select></p>';
	*/
	echo '<p><label for="project_order">Order :</label>';
	echo '<input type="text" maxlength="4" id="project_order" name="project_order" placeholder="Enter the Order" value="'.get_post_meta($argpost->ID,'project_order',true).'"></p>';
	
	echo '<p><label for="project_date">Year :</label>';
	echo '<input type="text" maxlength="4" id="project_date" name="project_date" placeholder="Enter the year" value="'.get_post_meta($argpost->ID,'project_date',true).'"></p>';
	...
	echo '<p><label for="project_client">Client :</label>';
	echo '<select id="project_client" name="project_client">';
	echo '<option value="">---Choose Client---</option>';
	while ($loop->have_posts() ) : $loop->the_post();
		echo '<option value="'.get_the_ID().'" '.(get_post_meta($argpost->ID,'project_client',true)==get_the_ID()?"selected":"").'>';
		the_title();
		echo'</option>';
	endwhile;
	echo '</select>';
	
	echo '<p><label for="project_movie">Movie :</label>';
	echo '<input type="text" id="project_movie" name="project_movie" placeholder="enter a url of moive" value="'.get_post_meta($argpost->ID,'project_movie',true).'" class="tcs_hp_project_input"></p>';

	echo '<p><label for="project_photo1">Photo1 :</label>';
	echo '<input type="text" id="project_photo1" name="project_photo1" placeholder="enter a url of photo" value="'.get_post_meta($argpost->ID,'project_photo1',true).'" class="tcs_hp_project_input"></p>';
	echo '<p><label for="project_photo2">Photo2 :</label>';
	echo '<input type="text" id="project_photo2" name="project_photo2" placeholder="enter a url of photo" value="'.get_post_meta($argpost->ID,'project_photo2',true).'" class="tcs_hp_project_input"></p>';
	echo '<p><label for="project_photo3">Photo3 :</label>';
	echo '<input type="text" id="project_photo3" name="project_photo3" placeholder="enter a url of photo" value="'.get_post_meta($argpost->ID,'project_photo3',true).'" class="tcs_hp_project_input"></p>';
	echo '<p><label for="project_photo4">Photo4 :</label>';
	echo '<input type="text" id="project_photo4" name="project_photo4" placeholder="enter a url of photo" value="'.get_post_meta($argpost->ID,'project_photo4',true).'" class="tcs_hp_project_input"></p>';

	echo '<div id="brand_div" class="tcs_hp_additional">';
	echo '<p><label for="project_task">Task :</label></p>';
	echo '<p><textarea id="project_task" name="project_task" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_task',true).'</textarea></p>';

	echo '<p><label for="project_idea">Idea :</label></p>';
	echo '<p><textarea id="project_idea" name="project_idea" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_idea',true).'</textarea></p>';

	echo '<p><label for="project_campaign">Campaign :</label></p>';
	echo '<p><textarea id="project_campaign" name="project_campaign" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_campaign',true).'</textarea></p>';

	echo '<p><label for="project_success">Sucess :</label></p>';
	echo '<p><textarea id="project_success" name="project_success" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_success',true).'</textarea></p>';

	echo '<p><label for="project_location">Location :</label></p>';
	echo '<p><textarea id="project_location" name="project_location" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_location',true).'</textarea></p>';
	echo '</div>';


	echo '<div id="bespoke_div" class="tcs_hp_additional">';
	echo '<p><label for="project_challenge">Challenge :</label></p>';
	echo '<p><textarea id="project_challenge" name="project_challenge" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_challenge',true).'</textarea></p>';

	echo '<p><label for="project_solution">Solution :</label></p>';
	echo '<p><textarea id="project_solution" name="project_solution" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_solution',true).'</textarea></p>';

	echo '<p><label for="project_achieve">How did we achieve this :</label></p>';
	echo '<p><textarea id="project_achieve" name="project_achieve" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_achieve',true).'</textarea></p>';

	echo '<p><label for="project_result">Result :</label></p>';
	echo '<p><textarea id="project_result" name="project_result" class="tcs_hp_project_textarea">'.get_post_meta($argpost->ID,'project_result',true).'</textarea></p>';
	echo '</div>';
}
...
function tcs_hp_project_additional_box_save( $post_id ) {
	...
	//project_date
	update_post_meta( $post_id, 'project_order', $_POST['project_order'] );
	//project_date
	update_post_meta( $post_id, 'project_date', $_POST['project_date'] );
	//project_client
	update_post_meta( $post_id, 'project_client', $_POST['project_client'] );
	//project_movie
	update_post_meta( $post_id, 'project_movie', $_POST['project_movie'] );
	
	//project_photo
	update_post_meta( $post_id, 'project_photo1', $_POST['project_photo1'] );
	update_post_meta( $post_id, 'project_photo2', $_POST['project_photo2'] );
	update_post_meta( $post_id, 'project_photo3', $_POST['project_photo3'] );
	update_post_meta( $post_id, 'project_photo4', $_POST['project_photo4'] );
